...
uncorrected_path = '/hdfs/store/user/ekoenig/MonoJet/PostFiles/%s/%s/20200417_roccor/'
correct_path = '/hdfs/store/user/ekoenig/MonoJet/PostFiles/%s/%s/20200416_update-tree-name/'

def compare():
    corrected = Region(autovar=True,show=False)
    # corrected = Region(autovar=True,show=False,path=correct_path%(corrected.year,corrected.region))
    uncorrected = Region(autovar=True,show=False,path=uncorrected_path%(corrected.year,corrected.region))
    def plot(variable,weight="weight"):
        for samples in (corrected,uncorrected):
            samples.initiate(variable)
            samples.setSumOfBkg()

        def plot_type(corr,uncorr,name):
            table = Table(["%s Yields"%variable,"RoccoR","Uncorrected","Ratio"])
            ...
            leg.AddEntry(corr,'RoccoR %s'%name,'pl')
            ...
            c.Update()
            SaveAs(c,"%s_%s"%(name.lower(),variable),year=samples.year,region=samples.region,sub="Compare_RoccoR")
