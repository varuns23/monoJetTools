...
int monoJetClass::getPhoIndex() {
  for (int ipho = 0; ipho < nPho; ipho++) {
    ...
    bool failIso = !CutBasedPFIsolation(ipho,"loose");
    bool IdIso = CutBasedPFPhotonID(ipho, "veryloose");

    if(kinematics && IdIso && failIso)
      return ipho;
  }
...
bool monoJetClass::CutBasedPFIsolation(Int_t ipho,TString phoWP) {
...
bool monoJetClass::CutBasedPFPhotonID(Int_t ipho, TString phoWP){
