...
void monoJetClass::Loop(Long64_t maxEvents, int reportEvery) {
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
    ...
    if(jetCand.size()>0){
      ...
      bool mindphijr = getMinDphiJR(pfMETPhi);
	...
	if(metcut < 0.5){
	  npfCaloCut++;
	  bool vetoElectrons = ele_veto(jetCand[0], 10.);
	  if(vetoElectrons){
	    nEleVeto++;

	    bool vetoMuons = muon_veto(jetCand[0],10.);
	    if (vetoMuons){
	      nMuVeto++;

	      bool vetoPhotons = pho_veto(jetCand[0], 15.);
	      if(vetoPhotons){
		nPhoVeto++;

		bool vetoTaus = tau_veto(jetCand[0]);
		if(vetoTaus){
		  nTauVeto++;
  ...
  h_cutflow->SetBinContent(3,nMinDphiJR);
  h_cutflow->SetBinContent(4,npfCaloCut);
  ...
  h_cutflow->SetBinContent(10,nRecoil);
}
...
bool monoJetClass::JetVetoDecision(int jet_index) {

  bool jetVeto=false;

  float dr_jet = deltaR(jetEta->at(jet_index), jetPhi->at(jet_index), muEta->at(jet_index), muPhi->at(jet_index));
...
bool monoJetClass::ele_veto(int jet_index,float elePtCut) {
  ...
  for(int i = 0; i < nEle; i++){
	...
	//Electron does not overlap jeton 
	double dR_Jet = deltaR(eleSCEta->at(i),eleSCPhi->at(i),jetEta->at(jet_index),jetPhi->at(jet_index));
...
bool monoJetClass::muon_veto(int jet_index,float muPtCut){
  bool decision = true;
  int countMu = 0; 
  ...
  for(int i = 0; i < nMu; i++) {
    ...
    //Muon overlap with jet
    double dR_jet = deltaR(muEta->at(i),muPhi->at(i),jetEta->at(jet_index),jetPhi->at(jet_index)); 

    if(kinematics && IdandIso && dR_jet > 0.4)
      countMu++;
  }
...
bool monoJetClass::pho_veto(int jet_index,float phoPtCut){
  ...
  for(int i = 0; i < nPho; i++){
    ...
    double dR_Jet    = deltaR(phoEta->at(i),phoPhi->at(i),jetEta->at(jet_index),jetPhi->at(jet_index));  

    if(kinematics && IdandIso && dR_Jet > 0.5)
      countPho++;
  }
...
bool monoJetClass::tau_veto(int jet_index){
  ...
  for(int i = 0; i < nTau; i++){
    ...
    double dR_Jet   = deltaR(tau_Eta->at(i), tau_Phi->at(i), jetEta->at(jet_index), jetPhi->at(jet_index));  
...
bool monoJetClass::getMinDphiJR(double met_phi){
  bool decision = false;
  ...
  for(int k=0; k < tmpJetlist.size(); k++){
    ...
    if(DeltaPhi(jetPhi->at(jc), met_phi) > 0.5) 
      count++;
  }
