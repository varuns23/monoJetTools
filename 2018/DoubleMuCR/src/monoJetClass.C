...
void monoJetClass::BookHistos(const char* outputFilename) {
  ...
  cutflow = new Cutflow(this,{s_TotalEvents,s_Triggers,s_TwoLooseMu,s_OneTightMu, s_OppCharge, s_ZMass,
	s_METFilters, s_HEMVeto,s_ElectronVeto,s_PhotonVeto,s_TauVeto,s_BJetVeto,s_minDPhiJetMET,s_dPFCaloMET,s_Recoil,s_JetSelection});
...
bool monoJetClass::UncLoop(float &event_weight) {
  ...
  if (!tau_veto(leadLepIndx,subleadLepIndx)) return false;
